...
class PairedTransformer(nn.Module):
    """
    处理配对时间序列的模型。
    ...
    def __init__(self, 
        ...
        super(PairedTransformer, self).__init__()
        


        transformer_input_size_A = feature_num_A
        transformer_input_size_B = feature_num_B 
        ...
        self.transformer_A = TransformerEncoder(
            ...
            dim_feedforward=dim_feedforward,
            dropout=dropout,
            max_len=seq_len_A  # 最大长度与FFT后的频率分量数量一致
        )
        ...
        self.transformer_B = TransformerEncoder(
            ...
            dim_feedforward=dim_feedforward,
            dropout=dropout,
            max_len=seq_len_B  # 最大长度与FFT后的频率分量数量一致
        )
    ...
    def forward(self, x1, x2):
        ...
        # 2. 拼接实部和虚部
        # 3. 通过Transformer A
        features_A = self.transformer_A(x1) # (batch, d_model)

        # --- 分支 B 处理 ---
        # 1. FFT
        # 2. 拼接实部和虚部
        # 3. 通过Transformer B
        features_B = self.transformer_B(x2) # (batch, d_model)
